...
const AddCategory=()=>{
    ...
    const goBack=()=>(
        <div className="mt-5">
        <Link className="btn btn-sm btn-primary mb-3" to="/admin/dashboard">Admin Home</Link>
        </div>
    )
...
if(success){
  ...
  const errorMessage=()=>{
    if(error){
        return <h4 className="text-danger">Failed to category</h4>
    }   
}    
...
<button className="btn btn-outline-info" onClick={onSubmit}>Create Category</button>
    ...
    return (
      <Base title="Create a category" description="Add a new category for new products" className="container bg-info p-4">
